    ...
    public Individual(Schedule schedule) {
        ...
        for (Studentgroup group : schedule.getGroupsAsArray()) {
            ...
            for (int courseId : group.getCourseIds()) {
                ...
                int randProfessor = course.getRandomProfessorId();
                if (randProfessor == -1) {
                    throw new BusinessException("001","Môn " + course.getCourseName() + " không có giảng viên nào!!");
                }
                newChromosome[chromosomeIndex] = randProfessor;
                chromosomeIndex++;
